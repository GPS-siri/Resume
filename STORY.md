...
상담사가 고객을 응대하기 위해 번호표(대기열 번호)를 뽀는 로직이 있었습니다.
이 로직은 단순했지만, 동시에 여러 상담사가 버튼을 누르면 같은 번호표가 중복 배정되는 Race Condition 문제가 발생할 수 있다는 사실을 당시에는 간과 하고 있었습니다.

기존 로직
![Counselor_Call_Base_Logic]()

- 고객 모델하우스 방문예약 -> 고객에게 알림톡으로 QR코드 및 예약 정보 전송 -> 모델하우스 상담원이 다음 대기 손님 중 한명 콜(상담 알림톡 보내기) -> 상담 진행 -> 다음 상담 대기 손님 콜 -> 반복

Race Condition 예시(출처: <https://junhyunny.github.io/information/operating-system/junit/race-condition/>)
![Race_Condition_image](assets/img/raceCondition1.png)

DB 조회와 배정이 순차적으로 처리되지 않은 것이 원인으로 동시에 요청이 들어오면 같은 번호표가 여러번 반환될 수 있는 확률이 충분히 있었습니다. 체감상 0.5초 내에 동시에 다음 상담 버튼을 눌러서 번호표 배정 로직을 타게 되면, 각 브라우저에서 해당 로직을 타고 API가 처리되는 시간 동안 한쪽이 번호를 배정받아 가기 전에 다수의 상담사가 똑같은 번호(대기순번 중 가장 위에 있는 번호)를 배정 받을 가능성이 충분히 존재했습니다.
실제로 이 0.5초 라는 시간은 생각보다 긴 시간이었으며 체감상 하루에 4만명이 방문하는 모델하우스에서 해당 현상이 4번 이상 발생했던 걸로 기억합니다.

당시의 저는 적은 경험으로 브라우저의 API 콜 속도와 서버단에서의 DB 처리 속도를 과신하는 경향이 있었던 것 같습니다.
이 과신은 위의 기능상 치명적은 문제를 야기 시켜 저에게 엄청난 교훈과 함께 많은 경험을 해주게 하는데, 지금 생각해보면 커리어를 시작한지 얼마 안된 1년차 개발자에게는 너무나도 가혹한 피드백들과 쓰디 쓴 경험들이 이었던 것 같습니다.
클라이언트의 매서운 피드백 전화는 저희팀을 정말 식은땀이 비처럼 오게 만들었습니다.
